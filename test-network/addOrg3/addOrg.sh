...
function generateOrgDefinition() {
  which configtxgen
  if [ "$?" -ne 0 ]; then # 如果上一条命令执行失败，则$?=0；若成功，则$?=1
...
function OrgUp () {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    DOCKER_SOCK=${DOCKER_SOCK} docker-compose -f docker/docker-compose-org${ORG_NUM}.yaml -f docker/docker-compose-couch-org${ORG_NUM}.yaml up -d 2>&1
  else
    DOCKER_SOCK=${DOCKER_SOCK} docker-compose -f docker/docker-compose-org${ORG_NUM}.yaml up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
...
COMPOSE_FILE_COUCH_ORG="docker/docker-compose-couch-org${ORG_NUM}.yaml"
# use this as the default docker-compose yaml definition
COMPOSE_FILE_ORG="docker/docker-compose-org${ORG_NUM}.yaml"
# certificate authorities compose file
COMPOSE_FILE_CA_ORG="docker/docker-compose-ca-org${ORG_NUM}.yaml"
# database
DATABASE="leveldb"
# organization
ORG_NUM = 3
